...
#if defined(NDEBUG)
# error "Fdpcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x857fa012105920b1eb21dd4386f4be025128957bdcc1bfc077848b10eaba8f96");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Fdpcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Fdpcoin Signed Message:\n";
...
{
    // Fdpcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Fdpcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Fdpcoin: 1 day
static const int64 nTargetSpacing = 5 * 60; // Fdpcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Fdpcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Fdpcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3f58d01a36b8aefcef753bc6bf982646490adc2a1477be0f4c9266a0b9fd5180");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Trempes tes couilles dans l'eau chaude";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c6e03b5616f4a50a4c10c5e0eba660c98b57e7a03bb7b8efff52c9f9bc143fe58b0dce4d94050f3af97f99cc7932b806c58bc53e02e0ac098cb6b1162271f339") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1518224529;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086531585;

        if (fTestNet)
        {
            block.nTime    = 1518224510;
            block.nNonce   = 386684667;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xe24738fa8c2fcf9414ad1aa8c02d076d390085dff843dae4c905dcf8569ee032"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Fdpcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FdpcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FdpcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FdpcoinMiner(CWallet *pwallet)
{
    printf("FdpcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fdpcoin-miner");
    ...
    try { loop {
        ...
        printf("Running FdpcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FdpcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FdpcoinMiner, pwallet));
}
